...
def validar_usuario(usuario, password): #Método
    db = sqlite3.connect("mensajeria.s3db") #Crea la conexión con la DB
    db.row_factory = sqlite3.Row #envía todas las cabeceras
    cursor = db.cursor() #Crea un apuntador
    consulta = "select * from usuarios where correo= '"+usuario+"' and password= '"+password+"' and estado = '1'" #Cadena de consulta
    cursor.execute(consulta)
    resultado = cursor.fetchall() #Envía todos los registros que encuentre
    return resultado
